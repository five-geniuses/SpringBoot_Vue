<template>
  <div class="orders-container">
            ...
            >
              <div class="item-content">
                ...
                <div class="item-actions">
                  ...
                  <!-- 修改订单按钮 - 仅在可修改状态下显示 -->
                  <el-button 
                    v-if="canEditOrder(orderGroup)"
                    type="warning" 
                    size="small"
                    @click="openEditOrderDialog(orderGroup)"
                  >
    ...
    <!-- 修改订单弹窗 -->
    <el-dialog
      v-model="editOrderVisible"
      title="修改订单信息"
      width="600px"
      destroy-on-close
      class="edit-order-dialog"
    >
      <div v-if="currentEditOrder" class="edit-order-form">
        <!-- 订单基本信息展示 -->
        <div class="order-basic-info">
          <h4>订单号：{{ currentEditOrder.orderNo }}</h4>
          <p>订单金额：¥{{ currentEditOrder.totalAmount }}</p>
        </div>

        <!-- 修改表单 -->
        <el-form :model="editOrderForm" label-width="100px" class="edit-form-content">
          <el-form-item label="收件人姓名" required>
            <el-input
              v-model="editOrderForm.receiverName"
          ...
          <el-form-item label="联系电话" required>
            <el-input
              v-model="editOrderForm.receiverPhone"
              placeholder="请输入联系电话"
              maxlength="20"
            />
          </el-form-item>
          <el-form-item label="收货地址" required>
            <el-input
              v-model="editOrderForm.receiverAddress"
              type="textarea"
              :rows="3"
...
const currentEditOrder = ref<GroupedOrder | null>(null)
...
// 判断是否可以修改订单
const canEditOrder = (orderGroup: GroupedOrder): boolean => {
  // 只有待处理（未支付）或待发货状态才能修改
  return orderGroup.orderState === 0 || (orderGroup.orderState === 1 && orderGroup.orderState <= 1)
}

// 打开修改订单弹窗
const openEditOrderDialog = (orderGroup: GroupedOrder) => {
  currentEditOrder.value = orderGroup
  editOrderForm.value = {
    receiverName: orderGroup.receiverName,
    receiverPhone: orderGroup.receiverPhone,
    receiverAddress: orderGroup.receiverAddress
  }
...
const isEditFormValid = computed(() => {
  return editOrderForm.value.receiverName.trim() !== '' &&
         editOrderForm.value.receiverPhone.trim() !== '' &&
         editOrderForm.value.receiverAddress.trim() !== ''
})

// 提交订单修改
const submitOrderEdit = async () => {
  if (!currentEditOrder.value || !isEditFormValid.value || !userId.value) return
  ...
  try {
    ...
    // 请求体只包含需要修改的字段
    const updateData = {
      receiverName: editOrderForm.value.receiverName.trim(),
      receiverPhone: editOrderForm.value.receiverPhone.trim(),
      receiverAddress: editOrderForm.value.receiverAddress.trim()
    }
    
    console.log('提交修改数据:', updateData)
    console.log('订单号:', currentEditOrder.value.orderNo)
    console.log('用户ID:', userId.value)
    
    // userId 作为查询参数，修改的数据放在请求体中
    const response = await fetch(`/api/api/orders/receiver/${currentEditOrder.value.orderNo}?userId=${userId.value}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    if (!response.ok) {
      ...
      console.error('修改订单失败详情:', errorData)
      throw new Error(errorData?.message || `修改订单失败 (${response.status})`)
    }
    ...
    if (result === true || result.success !== false) {
      ...
      throw new Error(result.message || '修改订单失败')
    }
    
  } catch (error) {
    console.error('修改订单失败:', error)
    ElMessage.error((error as Error).message || '修改订单失败，请稍后重试')
  } finally {
