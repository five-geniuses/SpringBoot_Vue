...
const loadTodayStats = async () => {
  try {
    // 获取今日订单数量
    const todayOrdersResponse = await axios.get('/api/api/orders/today')
    todayOrders.value = todayOrdersResponse.data.total || 0
    
    // 获取今日营业额
    const todayRevenueResponse = await axios.get('/api/api/orders/today/stats')
    todayRevenue.value = todayRevenueResponse.data.totalAmount || 0
  } catch (error) {
...
const loadOrders = async () => {
  ...
  try {
    ...
    const pageSize = 20 // 使用较小的分页大小，减少服务器压力
    let consecutiveErrors = 0 // 连续错误计数
    let maxRetries = 3 // 最大重试次数
    
    console.log('开始分页获取订单数据...')
    
    while (hasMoreData && consecutiveErrors < maxRetries) {
      try {
        ...
        console.log(`第${currentPage}页响应:`, response.data)
        
        let batchData = []
        let totalPages = 0
        let currentResponsePage = 0
        ...
        if (response.data.records && Array.isArray(response.data.records)) {
          ...
          currentResponsePage = response.data.current || 0
          ...
          // Spring Boot 分页格式
          batchData = response.data.content
          totalPages = response.data.totalPages || 0
          currentResponsePage = response.data.number || 0
        } else {
        ...
        // 过滤重复订单
        const uniqueOrders = batchData.filter((order: any) => {
        ...
        console.log(`第${currentPage}页获取到${batchData.length}条数据，去重后${uniqueOrders.length}条，累计${allOrdersTemp.length}条`)
        ...
        if (batchData.length === 0) {
          console.log('当前页无数据，停止请求')
          hasMoreData = false
        } else if (totalPages > 0 && currentPage >= totalPages) {
          console.log('已到达最后一页，停止请求')
          hasMoreData = false
        } else if (batchData.length < pageSize) {
          console.log('当前页数据少于分页大小，可能是最后一页')
          hasMoreData = false
        } else {
          currentPage++
        }
        ...
        // 安全限制：最多请求200页
        if (currentPage >= 200) {
          console.log('已达到最大页数限制，停止请求')
          ElMessage.warning('数据量过大，已加载前4000条记录')
          break
        }
        ...
        consecutiveErrors++
        console.error(`第${currentPage}页请求失败 (${consecutiveErrors}/${maxRetries}):`, error)
        
        if (error.response?.status === 500) {
          ElMessage.warning(`第${currentPage}页加载失败，正在重试...`)
          
          // 如果是500错误，尝试减小分页大小
          if (pageSize > 10) {
            console.log('尝试减小分页大小...')
            return await loadOrdersWithSmallerPageSize(allOrdersTemp, currentPage, 10)
          }
        } else if (error.response?.status === 404) {
          console.log('API路径可能不正确，停止请求')
          break
        ...
        // 如果不是第一页失败，继续尝试下一页
        if (currentPage > 0 && consecutiveErrors < maxRetries) {
          currentPage++
          continue
        } else if (currentPage === 0) {
          // 第一页就失败，使用降级方案
          console.log('第一页请求失败，尝试降级方案...')
          break
        }
      }
    }
    
    console.log(`订单数据获取完成，共${allOrdersTemp.length}条`)
    allOrders.value = allOrdersTemp
    
    // 显示加载结果
    if (allOrdersTemp.length > 0) {
      ElMessage.success(`成功加载${allOrdersTemp.length}条订单数据`)
...
// 降级方案：使用更小的分页大小
const loadOrdersWithSmallerPageSize = async (existingOrders: any[], startPage: number, smallerPageSize: number) => {
  console.log(`使用更小分页大小${smallerPageSize}继续加载...`)
  
  let currentPage = startPage
  let hasMoreData = true
  const processedOrderNos = new Set(existingOrders.map(order => order.orderNo))
  
  while (hasMoreData && currentPage < 100) {
    try {
      const response = await axios.get('/api/api/orders/admin/list', {
        params: { page: currentPage, size: smallerPageSize },
        timeout: 15000
      })
      ...
      const uniqueOrders = batchData.filter((order: any) => !processedOrderNos.has(order.orderNo))
      existingOrders.push(...uniqueOrders)
      uniqueOrders.forEach((order: Order) => {
  processedOrderNos.add(order.orderNo)
})
      
      console.log(`小分页第${currentPage}页获取${uniqueOrders.length}条新数据`)
      
      if (batchData.length < smallerPageSize) {
        hasMoreData = false
      } else {
        currentPage++
        await new Promise(resolve => setTimeout(resolve, 300)) // 更长延迟
      }
      
    } catch (error) {
      console.error(`小分页第${currentPage}页失败:`, error)
      break
    }
  }
  
  allOrders.value = existingOrders
  ElMessage.success(`最终加载${existingOrders.length}条订单数据`)
...
const fetchOrdersWithItems = async () => {
  ...
  console.log(`开始获取${orderNos.length}个订单的商品信息...`)
  ...
  for (let i = 0; i < orderNos.length; i += batchSize) {
    ...
    // 显示进度
    if (i % 15 === 0) {
      console.log(`商品信息获取进度: ${Math.min(i + batchSize, orderNos.length)}/${orderNos.length}`)
    }
  }
  
  console.log('订单商品信息获取完成')
}
